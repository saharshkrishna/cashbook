...
const CashoutModal = ({ onClose, type, onSubmit, initialData }) => {
  ...
  const [entryType, setEntryType] = useState("");
    const { parties, categories, paymentModes, loading, } = useContext(DataContext);
  ...
  const handleAddSuccess = (newEntry) => {
    if (entryType === "category") {
      setFormData((prev) => ({ ...prev, category: newEntry }));
    } else if (entryType === "paymentMode") {
      setFormData((prev) => ({ ...prev, paymentMode: newEntry }));
    }
  };
        ...
        <div className="overflow-y-auto h-[calc(100vh-80px)] p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* Category Selection */}
              <div>
                <label className="block text-sm font-small text-gray-700">Category</label>
                <div className="flex gap-2">
                <select
              ...
              <div>
                <label className="block text-sm font-small text-gray-700">Payment Mode</label>
                <div className="flex gap-2">
                <select
                value={formData.paymentMode}
                placeholder="Add paymentmode"
...
CashoutModal.propTypes = {
  ...
  onSubmit: PropTypes.func.isRequired,
   initialData: PropTypes.object, // Add initialData prop
};
