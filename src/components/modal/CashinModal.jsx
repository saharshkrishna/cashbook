...
const CashinModal = ({ onClose, type, onSubmit, initialData, }) => {
  const [formData, setFormData] = useState({
    ...
    time: initialData?.time || new Date().toLocaleTimeString("en-US", {
      hour: "2-digit",
      minute: "2-digit",
      hour12: true,
    }),
    ampm: initialData?.ampm || (new Date().getHours() >= 12 ? "PM" : "AM"),
  ...
  const [addingModalType, setAddingModalType] = useState("");
  ...
  // Update the onChange handler to store files in state
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-end z-40">
      <div className="bg-white w-[400px] h-full overflow-hidden">
